...
// (DESIGN.md 8.4) — six shapes, each its own gentle drift/rotate/bob/sway so
// none of them sync up. Pure CSS keyframes, no JS; disabled outright under
// prefers-reduced-motion rather than just slowed. pointer-events-none and
// aria-hidden since these carry no content.
    ...
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .dp-shape-drift, .dp-shape-bob, .dp-shape-sway, .dp-shape-spin { animation: none; }
        }
      `}</style>
      ...
      <svg className="dp-shape-drift absolute bottom-[10%] right-[4%] size-16" viewBox="0 0 64 20" fill="none">
        <path d="M2 18 L14 2 L26 18 L38 2 L50 18 L62 2" strokeWidth="4" strokeLinecap="round" className="stroke-memphis-sky" />
      </svg>

      <svg className="dp-shape-bob absolute top-[66%] right-[24%] size-10" viewBox="0 0 40 40" fill="none">
