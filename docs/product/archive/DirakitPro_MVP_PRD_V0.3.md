...
21. [Appendix A — Suggested MVP Course Shape](#appendix-a--suggested-mvp-course-shape)
...
| Catalog    | Course catalog/detail, outcome preview, pricing, active bundle campaign. |
| Identity   | Register, login, logout, Google login, email verification/reset, protected routes. |
| Commerce   | Direct course purchase, fixed/choose-N bundles, checkout, order/payment, enrollment activation. |
...
**4.** System creates Order with immutable course/price snapshot.
...
**1.** Guest opens an ACTIVE bundle campaign.
...
**5.** System creates Order with immutable bundle snapshot and the exact courses that will be granted after payment.
...
**9.** Learner sees all newly acquired courses on the dashboard.
...
**3.** Submission enters SUBMITTED/COMPLETED state according to MVP rules.
...
> Admin dapat menyimpan course sebagai DRAFT, PUBLISHED, atau UNPUBLISHED. Hanya PUBLISHED tersedia untuk guest.
...
> Guest dapat melihat promotional bundle yang ACTIVE dan berada dalam campaign window. Bundle harus menjelaskan type, harga bundle, course eligible/included, dan rule pemilihan bila `CHOOSE_N`.

**CAT-006 Course ownership indicator \[P0\]**

> Pada catalog, course detail, dan bundle selector, authenticated learner dapat melihat course yang sudah dimiliki sehingga tidak membeli course yang sama secara tidak sengaja.
...
>
> **Acceptance:** Satu successful direct course purchase memberikan learner akses ke seluruh stage, lesson, build task, checkpoint, resource, dan final project dalam course tersebut sesuai access rule course.
...
> Bundle memiliki harga sendiri yang independen dari total harga individual course dan dapat memiliki `starts_at`/`ends_at`. Bundle hanya dapat dibeli ketika ACTIVE dan berada di dalam campaign window.
...
> Order bundle menyimpan exact course IDs/titles yang akan diberikan setelah pembayaran. Perubahan isi bundle setelah order dibuat tidak boleh mengubah grant pada historical order.
...
> Domain mendukung status REFUNDED walaupun workflow refund otomatis dan automatic entitlement revocation tidak wajib pada MVP.
...
> Hanya learner dengan valid enrollment dapat membuka paid course content. Enrollment terhadap satu course memberikan akses ke seluruh content course tersebut; tidak ada paywall per module/video pada MVP.
...
> Enrollment dapat dikaitkan dengan learner Project untuk course tersebut.

**PRJ-002 Project submission \[P0\]**

> Learner dapat mengirim live URL, optional repository URL, screenshot, dan notes. Final project submission merupakan salah satu syarat course completion.

**PRJ-003 Project visibility \[P0\]**

> Learner dapat menetapkan project `PRIVATE` atau `PUBLIC`. Publication selalu merupakan pilihan learner dan bukan syarat course completion.
...
> Perubahan sensitif admin minimum pada course publish/pricing/bundle/order/project moderation dicatat dalam AdminAuditLog.
...
Minimum states: PENDING → PAID; alternative terminal/exception states: EXPIRED, CANCELLED, REFUNDED. State change harus audit-friendly dan idempotent. Order harus menyimpan `DIRECT_COURSE` atau `BUNDLE` sebagai purchase source/type.

### 10.2 Payment state

Minimum normalized states: PENDING, SETTLEMENT/PAID, FAILED, EXPIRED, REFUNDED. Raw provider status tetap dapat disimpan untuk debugging/audit.

### 10.3 Bundle state

Minimum lifecycle: DRAFT, ACTIVE, INACTIVE, EXPIRED. `ACTIVE` tetap harus memenuhi campaign window `starts_at <= now <= ends_at` jika kedua batas waktu dikonfigurasi.
...
Minimum states: ACTIVE, COMPLETED, REVOKED. Paid enrollment dibuat hanya setelah payment successful; free enrollment dapat langsung ACTIVE. Satu Enrollment selalu merepresentasikan akses learner terhadap satu Course, terlepas apakah course diperoleh lewat direct purchase, bundle, atau free enrollment.
...
Project workflow minimum: DRAFT → SUBMITTED → COMPLETED. Visibility `PRIVATE`/`PUBLIC` dipisahkan dari workflow state. Moderation state dipisahkan lagi menjadi `UNREVIEWED`, `APPROVED`, `REJECTED`, `HIDDEN`; featuring menggunakan flag/state tersendiri.
...
3. final Project Submission telah `SUBMITTED`/valid menurut rule MVP.
...
- Course yang sudah dimiliki learner tidak dapat dipilih ulang di `CHOOSE_N`.
...
| OrderCourseGrant | Immutable list of exact course(s) to enroll after successful payment. |
| Payment | Provider transaction status, references, normalized state. |
| Project | Learner output linked to user/course/enrollment; visibility/moderation/featured state. |
| ProjectSubmission | Live URL, repository URL, notes, final submission state. |
| ProjectAsset | Screenshot/media references in object storage. |
| AdminAuditLog | Administrative mutation audit including course pricing, bundles, payments, moderation. |
...
Event properties minimum harus memungkinkan funnel per course dan purchase type (`DIRECT_COURSE`/`BUNDLE`), bundle/campaign identifier bila relevan, selected/granted course IDs, source/campaign attribution, user/enrollment identifier yang aman, lesson/stage/milestone identifier, price/order identifier, dan timestamp.

### 13.3 Primary funnel

Primary learning funnel: Course viewed → Checkout started → Payment completed → Course started → 50% build reached → Course completed → Project published → Project shared. Commerce analysis harus dapat membandingkan direct-course conversion versus bundle-campaign conversion.
...
- Enrollment creation/access control.
...
**2.** Open `CHOOSE_N` bundle → select exactly N eligible unowned courses → checkout → payment success → N idempotent enrollments.
...
### 18.3 Resolved decisions for Product Lock Candidate

| Decision | V0.3 decision |
|---|---|
| Product monetization | **Individual course purchase is primary.** Buying one course unlocks all content in that course. |
| Course pricing | **Admin-configurable per course.** No hardcoded global launch price in product rules. |
| Bundle campaign | **P0.** Supports `FIXED` and `CHOOSE_N`; bundle price is independent of retail total. |
...
| Course completion | All REQUIRED lessons + required Build Milestones + final Project Submission. Publication and human review are not required. |
...
- Paid course dapat dibeli secara direct melalui Midtrans dan satu successful purchase mengaktifkan satu course enrollment secara idempotent.

- Admin dapat membuat minimal satu `FIXED` dan satu `CHOOSE_N` bundle; successful bundle purchase membuat exact one-to-many course enrollments dari immutable grant snapshot.

- Learner dapat membuka course workspace, menyelesaikan lesson/checkpoint, dan melihat Build Progress.

- Learner dapat memenuhi completion rule: seluruh REQUIRED lessons + required Build Milestones + final Project Submission; project boleh tetap PRIVATE.
...
- Core product events terekam sehingga funnel Purchase → Build → Publish dapat dianalisis.
...
- Dokumentasi runbook/setup minimum tersedia untuk developer dan deployment.

> **Product lock recommendation**
>
> V0.3 ini adalah **Product Lock Candidate**. Setelah satu final consistency audit terhadap requirement IDs, routes, states, data model, analytics, dan Definition of Done, product scope dapat dinaikkan menjadi **“DirakitPro MVP PRD V1.0 — LOCKED”**. Legal/digital clearance DirakitPro tetap menjadi **Public Brand Release Gate**, bukan alasan membuka kembali scope produk. Setelah V1.0 lock, perubahan P0 harus melalui explicit scope-change decision.
